...
void do_proxy(ws_ctx_t *ws_ctx, np_socket target) {
    fd_set rlist, wlist, elist;
    struct timeval tv;
    //int i, maxfd, client = ws_ctx->sockfd;
	int i;
	np_socket maxfd, client = ws_ctx->sockfd;
...
int main(int argc, char *argv[])
{
	...
	settings.cert = _fullpath(NULL, "self.pem",_MAX_PATH );
    if (!settings.cert) {
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'c':
				...
				settings.cert = _fullpath(NULL, optarg,_MAX_PATH );
                if (! settings.cert) {
            ...
            case 'k':
				...
				settings.key = _fullpath(NULL, optarg,_MAX_PATH );
                if (! settings.key) {
