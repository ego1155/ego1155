...
ws_ctx_t *ws_socket(ws_ctx_t *ctx, np_socket socket) {
    ctx->sockfd = socket;
    return ctx;
}

//ws_ctx_t *ws_socket_ssl(ws_ctx_t *ctx, int socket, char * certfile, char * keyfile) {
ws_ctx_t *ws_socket_ssl(ws_ctx_t *ctx, np_socket socket, char * certfile, char * keyfile) {
    int ret;
    char msg[1024];
    char * use_keyfile;
...
ws_ctx_t *do_handshake(np_socket sock) {
